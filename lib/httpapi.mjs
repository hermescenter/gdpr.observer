...
import { getDBData, getDBCampaign, getDBBatches } from './database.mjs';
...
async function getCampaign(db, req, res) {
  /* this function simply pull the information available as input */
  const input = await getDBCampaign(db, req.params);
  return input;
}
...
export async function serveCampaignData(db, expressApp) {
  expressApp.get('/api/campaign/:name', cors(), async (req, res) => {
    try {
      const f = _.partial(getCampaign, db);
      const retval = await f(req, res);
      res.json(retval);
    } catch(error) {
      handleError(error, req, res, "getCampaign");
    }
  });
}
...
export const routes = [ serveDataByBatch, serveCampaignData, getBatchList];
