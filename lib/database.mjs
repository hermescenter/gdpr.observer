...
export async function getDBBatches(db) {
  ...
  let retv = { collection: {}, country: {} }, uniq = [];
  ...
  try {
    for(const cname of sources) {
       ...
       _.set(retv.collection, cname, l);
       uniq = _.uniq(_.concat(uniq, l));
    }

    debug("Unique countries: %j", uniq);
    ...
    for(const country of uniq) {
       ...
       _.set(retv.country, country, _.first(d).when);
    }
